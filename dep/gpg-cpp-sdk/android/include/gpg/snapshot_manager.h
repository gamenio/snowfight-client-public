...
class GPG_EXPORT SnapshotManager {
 public:
  ...
  /**
   * Asynchronously reads a snapshot from the disk, and copies it into memory.
  ...
  /**
   * Synchronously reads a snapshot from the disk, and copies it into memory.
   * The data is passed back by value for easier modification. Each call to this
   * function results in a full read. This means that it is typically best only
  ...
  /*
   * Discards the contents of the snapshot and closes the contents. This will
   * discard all changes made to the file, and close the snapshot to future
   * changes until it is re-opened. The file will not be modified on the server.
   *
   * Usually there is no need to call this method explicitly as it would be
   * called automatically when the last SnapshotMetadata referencing an
   * uncommitted snapshot is destroyed.
   *
   * This method does nothing if the snapshot has already been closed.
   */
  void DiscardAndClose(const SnapshotMetadata &snapshot_metadata);
