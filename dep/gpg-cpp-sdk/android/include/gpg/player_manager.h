...
class GPG_EXPORT PlayerManager {
 public:
   ...
   * @ingroup Callbacks
   */
  typedef std::function<void(const FetchSelfResponse &)> FetchSelfCallback;
   ...
   * calling <code>Fetch(DataSource data_source, const std::string& player_id,
   ...
   * can be retrieved by re-requesting the given player through the
   * {@link Fetch} API.
