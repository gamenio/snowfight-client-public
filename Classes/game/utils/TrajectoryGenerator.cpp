...
void BezierCurveConfig::draw(DrawNode* drawNode) const
{
	switch (order)
	{
	case QUADRATIC:
	{
		Point startPos = startPosition;
		Point control = startPosition + controlPoints[0];
		Point endPos = startPosition + endPosition;
		drawNode->drawQuadBezier(startPos, control, endPos, 360, Color4F::GREEN);
		drawNode->drawLine(endPos, control, Color4F::MAGENTA);
		break;
	}
	case CUBIC:
	{
		Point startPos = startPosition;
		Point control1 = startPosition + controlPoints[0];
		Point control2 = startPosition + controlPoints[1];
		Point endPos = startPosition + endPosition;
		drawNode->drawCubicBezier(startPos, control1, control2, endPos, 360, Color4F::GREEN);
		drawNode->drawLine(startPos, control1, Color4F::MAGENTA);
		drawNode->drawLine(endPos, control2, Color4F::MAGENTA);
		break;
	}
	default:
		break;
